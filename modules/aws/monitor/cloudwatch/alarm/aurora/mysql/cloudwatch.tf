...
resource "aws_cloudwatch_metric_alarm" "aurora_auroravolumebyteslefttotal" {
  ...
  alarm_name          = "${each.value.aurora_cluster_name}-AuroraVolumeBytesLeftTotal${var.sfx}"
  comparison_operator = "LessThanOrEqualToThreshold"
  ...
  alarm_description   = "${each.value.aurora_cluster_name}-AuroraVolumeBytesLeftTotal${var.sfx}"
...
resource "aws_cloudwatch_metric_alarm" "aurora_dmllatency" {
  ...
  alarm_name          = "${each.value.aurora_cluster_name}-DMLLatency${var.sfx}"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  ...
  alarm_description   = "${each.value.aurora_cluster_name}-DMLLatency${var.sfx}"
